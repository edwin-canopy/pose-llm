...
# Root convention for the kinematic-tree root joints (nose + both shoulders).
#   False (default): per-frame mid-shoulder root -> translation-invariant features.
#   True:            fixed frame-0 mid-shoulder root -> those joints carry body
#                    translation/drift; global_state stores the constant frame-0 root.
# Env POSE_FIRST_FRAME_ROOT=1 flips the default for all loaders (training included).
FIRST_FRAME_ROOT_DEFAULT = os.environ.get("POSE_FIRST_FRAME_ROOT", "0").lower() not in ("0", "", "false", "no")


def set_first_frame_root(enabled: bool) -> None:
    """Set the process-wide root convention, overriding the env-derived default.

    Called from build_dataloaders with ``TrainConfig.first_frame_root`` so the
    YAML config is authoritative. Also exports POSE_FIRST_FRAME_ROOT so
    DataLoader workers (and any subprocess) re-derive the same value on import.
    """
    global FIRST_FRAME_ROOT_DEFAULT
    FIRST_FRAME_ROOT_DEFAULT = bool(enabled)
    os.environ["POSE_FIRST_FRAME_ROOT"] = "1" if enabled else "0"
...
def _process_poses_to_clip(
    ...
    first_frame_root: bool | None = None,
    ...
    `first_frame_root` (None -> FIRST_FRAME_ROOT_DEFAULT / env POSE_FIRST_FRAME_ROOT):
    if True, the kinematic root joints (PARENT_INDEX == -1: nose + both shoulders)
    are offset from the FIXED frame-0 mid-shoulder instead of each frame's own, and
    global_state stores that constant root (so position = root + offset still inverts).
    """
    if first_frame_root is None:
        first_frame_root = FIRST_FRAME_ROOT_DEFAULT
    T = poses.shape[0]
    if T == 0:
        return None
    ...
    _, kc_keypoints = positions_to_offsets(positions)      # (T, NUM_JOINTS, 2)
    if first_frame_root:
        root0 = root[0]
        for i in range(NUM_JOINTS):
            if PARENT_INDEX[i] == -1:
                kc_keypoints[:, i, :] = positions[:, i, :] - root0
        root_out = np.broadcast_to(root0, root.shape)      # constant frame-0 root
    else:
        root_out = root
    kc_keypoints /= median_sw
    keypoints_flat = kc_keypoints.reshape(T, -1)           # (T, 2*NUM_JOINTS)
    ...
    global_state = np.stack(
        [root_out[:, 0], root_out[:, 1], shoulder_width], axis=-1
    )  # (T, 3)
